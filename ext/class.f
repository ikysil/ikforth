...
: GET/CREATE-MEMBER-ID (S SLITERAL"name" members-wordlist -- member-id )
  ...
  SEARCH-WORDLIST 0= IF
                       ...
                       GET-CURRENT >R
                       SET-CURRENT
                       (DO-CREATE) -ROT VEF-USUAL CHECK-HEADER, 
                       R> SET-CURRENT
                     ELSE
                       >R DROP DROP DROP R>
                     THEN ;
...
: CREATE-FIELD  (S size -- field-CFA )
  (DO-CREATE) 0 0 VEF-USUAL HEADER, >R
  CURRENT-CLASS >SIZE DUP @ , +! R> DOES> @ + ;

: FIELD (S size "name" -- )
  ?CLASS CREATE-FIELD
  MF-FIELD MF-NAME OR SWAP
  BL WORD COUNT GET/CREATE-FIELD-ID CURRENT-CLASS CLASS-ADDMEMBER ;

: M-ID: (S method-id -- flags method-id xt )
  ?CLASS MF-METHOD SWAP :NONAME ;

: M: (S "name" -- flags method-id xt )
  ?CLASS MF-METHOD MF-NAME OR BL WORD COUNT GET/CREATE-METHOD-ID :NONAME ;

: ;M (S flags method-id xt -- )
  ?CLASS POSTPONE ; SWAP CURRENT-CLASS CLASS-ADDMEMBER ; IMMEDIATE/COMPILE-ONLY

: GET-MEMBER-ID (S SLITERAL"name" members-wordlist -- member-id )
  OVER 0= IF EXC-EMPTY-NAME THROW THEN
  SEARCH-WORDLIST 0= IF EXC-UNDEFINED THROW THEN ;
...
: SUPER-INVOKE (S instance "name" -- )
  ...
  POSTPONE LITERAL POSTPONE (ID>CFA-INVOKE) ; IMMEDIATE/COMPILE-ONLY

: PARSE-GET-FIELD-ID (S "name" -- field-id )
  BL WORD COUNT GET-FIELD-ID ;
...
0 0 (CLASS) OBJECT
  ...
  M: .NAME THIS CELL- BODY> >NAME COUNT TYPE ;M
...
\  M: SEE
\     BL WORD COUNT GET-METHOD-ID THIS CLASSOF ID>CFA (SEE)
\  ;M
